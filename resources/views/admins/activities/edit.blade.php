...
<div class="container-fluid px-4">
    ...
    <form action="{{ route('admins.activities.update',$activity->id) }}" method="POST" role="form" enctype="multipart/form-data">
        @method('PATCH')
        @csrf
        <div class="form-group">
            <label for="title" class="form-label">活動名稱</label>
            <input id="title" name="title" type="text" class="form-control" value="{{ old('title',$activity->title) }}">
        </div>
        <div class="form-group">
            <label for="content" class="form-label">活動內容</label>
            <textarea id="editor" name="content" class="form-control">{!! old('content',$activity->content) !!}</textarea>
        </div>
        <div class="d-flex justify-content-end mt-3">
            <div class="me-4">
                <button type="submit" name="status" value="0" class="btn btn-primary btn-sm">暫存活動</button>
            </div>
            <div>
